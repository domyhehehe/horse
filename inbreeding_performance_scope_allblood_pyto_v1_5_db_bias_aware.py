...
VERSION = "1.5.0-batch-exact-db-bias-aware"
...
def main():
    ...
    meta={"version":VERSION,"blood":str(blood),"stakes":[str(x) for x in stakes],"scope":scope,"birth_start":ystart,"birth_end":yend,"all_blood_output":True,"no_N_cutoff":True,"grade_start":gs,"grade_rule":"pre-grade era -> NA; grade field only; series_grade never used as grade","db_bias_aware":True,"stakes_absence_semantics":"not observed in source stakes CSV / not equivalent to raced-and-lost","rank_method":"ALL_BLOOD reference; average-tie midrank; exact zero separated for Fall/F5/DeepDelta bins"}
            ...
            for metric in metrics:
                for outcome in outcomes:
                    ...
                    w.writerow({
                        ...
                        "VertexSD":v,"GroupedCells":len(groups),
                        "Note":"生産国×出生5年帯を基準に50順位cellへ集約。DB内観測モデルであり公式競走成功確率ではない。Vertexは最適近交値ではない"
                    })
...
if __name__ == "__main__":
    if "--self-test" in sys.argv:
        self_test_all_in_one_v15()
    else:
        print("="*76)
        print("近交係数 × 競走成績 BATCH EXACT DB-BIAS-AWARE ALL-IN-ONE", VERSION)
        print("単体ファイル: Exact F row-L batch + sibling pair reuse + F5/5代pair cache + stakes永続cache")
        print("ALL_BLOOD基準midrank / true stakes source presence / DB coverage audit / JPN_ALL・JRA・NAR")
        print("="*76)
        main()
